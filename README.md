...
Need to replace untrasonic/lidar with depth field camera - tof camera
...
a daemon at startup while testing.

We are switching to tof camera testing. I2C now only used for motor control or servos if needed.
...
filer
    Save and read speed data for hardware to use.

map - unused currently
    Search and map functions to help determine location and obstacles
    
commands
    Start thread to parse and execute commands from app over WiFi.
manager - unused
